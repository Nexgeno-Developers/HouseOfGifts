...
(function ($) {
    ...
    function hasEdits() {
        return (editedItems !== null && Object.keys(editedItems).length > 0) ||
    ...
    function addItemToTable(productData) {
        ...
        var statusText = isSufficient ? opsdeskLang.sufficient : opsdeskLang.insufficient;

        html += '<tr data-combo-item-id="' + comboItemId + '" style="background-color: #fffacd;">';
        ...
        html += '<input type="number" class="form-control opsdesk-qty-input" value="' + 
            formatNumber(requiredQty) + '" min="0" step="1" data-item-id="' + 
            comboItemId + '" data-qty-per-unit="' + qtyPerUnit + '">';
        html += '</div>';
        html += '</td>';
        html += '<td class="text-center"><span class="label ' + statusClass + '">';
        html += '<i class="fa ' + statusIcon + '"></i> ' + statusText + '</span></td>';
        html += '<td class="text-center">';
        html += '<button type="button" class="btn btn-xs btn-danger opsdesk-remove-item" data-item-id="' + 
            comboItemId + '">';
        ...
        if (itemCount > 0) {
            ...
            if (allSufficient) {
                summaryLabel
                    ...
                    .html('<i class="fa fa-check-circle"></i> ' + opsdeskLang.fulfillable);
            } else {
                summaryLabel
                    .removeClass('label-success')
                    .addClass('label-danger')
                    .html('<i class="fa fa-exclamation-circle"></i> ' + opsdeskLang.not_fulfillable);
            }
        } else {
    ...
    function renderTable(data) {
        ...
        if (components.length === 0) {
            ...
            $.each(components, function (i, row) {
                ...
                html += '<div class="input-group" style="width: 120px; margin: 0 auto;">';
                html += '<input type="number" class="form-control opsdesk-qty-input" value="' + 
                    formatNumber(row.required_quantity) + '" min="0" step="1" data-item-id="' + 
                    row.combo_item_id + '" data-qty-per-unit="' + row.quantity_per_unit + '">';
                html += '</div>';
                html += '</td>';
                html += '<td class="text-center"><span class="label ' + statusClass + '">';
                html += '<i class="fa ' + statusIcon + '"></i> ' + statusText + '</span></td>';
    ...
    function attachRowHandlers() {
        $('.opsdesk-qty-input').off('input change').on('input change', function () {
            var itemId = $(this).data('item-id');
            var newQty = parseFloat($(this).val()) || 0;
            var availableStock = parseFloat($(this).closest('tr').find('td:eq(2)').text().replace(/,/g, '')) || 0;
        ...
        $('.opsdesk-remove-item').off('click').on('click', function () {
            var itemId = $(this).data('item-id');
            removeItem(itemId);
        });
    }
    ...
    function updateItemStatus(itemId, requiredQty, availableStock) {
        ...
        statusCell.html(
            ...
            '<i class="fa ' + statusIcon + '"></i> ' + statusText + '</span>'
        );
    }
    ...
    $(function () {
        ...
        $('#opsdesk_seed_stock_btn').on('click', function () {
            if (!confirm('Update all inventory items with random stock (10-500)? This cannot be undone.')) {
                return;
            }
            ...
            $.post(opsdeskAjaxUrl.replace('ajax_availability', 'seed_random_stock'), postData)
