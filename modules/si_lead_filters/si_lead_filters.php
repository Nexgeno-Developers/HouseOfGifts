...
define('SI_LEAD_FILTERS_MAX_CUSTOM_FIELDS', 4);
 ...
 */
register_uninstall_hook(SI_LEAD_FILTERS_MODULE_NAME, 'si_lead_filters_uninstall_hook');

function si_lead_filters_uninstall_hook()
{
    $CI = &get_instance();
...
function si_lead_filters_init_menu_items()
{
	...
	if (is_admin() || has_permission('si_lead_filters', '', 'view')) {
		...
		$default_template_id = $CI->si_lead_filter_model->get_default_template(get_staff_user_id());
		...
		$CI->app_menu->add_sidebar_children_item('lead-filters', [
			...
			'href'		=> admin_url('si_lead_filters/leads_filter' . ($default_template_id > 0 ? '?filter_id='.$default_template_id : "")),
			'position'	=> 1,
		]);
		$CI->app_menu->add_sidebar_children_item('lead-filters', [
