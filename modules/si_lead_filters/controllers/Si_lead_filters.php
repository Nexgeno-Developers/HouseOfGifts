...
class Si_lead_filters extends AdminController 
{
	...
	private function get_leads_summary()
	{
		...
		if($result) {
			...
			foreach($result as $_status) {
				if($_status['status']==0) {
					...
					$total_lead_value += ($perfex_version >= 250 ? $_status['total_value'] : 0);
		...
		$customer_amount = isset($vaule_count[1]) ? $vaule_count[1] : 0;
		...
		array_unshift($statuses, [
			...
			'total' => app_format_money($customer_amount, get_base_currency()),
			'color' => '#4caf50',
		]);

		array_unshift($statuses, [
			'name'  => 'Total Lead Value',
			'total' => app_format_money($total_lead_value, get_base_currency()),
			'color' => '#2196f3', // blue color or choose another
		]);
